  ...
  const genres = [ //this CANNOT be random. It must be the same in order to properly vector. I mean I guess you can randomize it and then assign it this order, but why? 
    ...
    async function generateGenreVector(genre, userVector) {
      // Check if userVector is not undefined
      if (!userVector) {
      console.error("User vector is undefined.");
      return Array(genres.length).fill(0);
      } 
    ...
    window.generateRecommendations = async function(){ 
      chrome.storage.sync.get('userVector', async function (data) { //get userVector saved to web storage. Will implement local later.
        ...
        if (!userVector) { // Check if the userVector is undefined, and set it to an array filled with zeros as a fallback
          console.error("User vector not found in Chrome storage. Using an empty vector instead.");
          userVector = Array(genres.length).fill(0);
        }  
